...
function LoginBox() {
    ...
    const { login, isAuthenticated } = useContext(AuthContext); // Usa useContext
    const navigate = useNavigate();
    // Redirige a /chat si ya está autenticado
    useEffect(() => {
        console.log('Login - useEffect - isAuthenticated:', isAuthenticated);
        if (isAuthenticated) {
            console.log('Login - Redirigiendo a /chat por isAuthenticated');
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (token) {
                ...
                login(token); // Pasa userId al AuthContext si lo necesitas
